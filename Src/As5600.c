...
void AS5600_Init(){
	...
	HAL_I2C_Mem_Write(&hi2c1, AS5600_SLAVE_ADDRESS , AS5600_REGISTER_CONF_HIGH, 1, &Data, 1, 100);
	Data = 0;
	Data = Data|(AS5600_FAST_FILTER_DEFAULT);
	Data = Data|(AS5600_SLOW_FILTER_DEFAULT<<2);
	Data = Data|(AS5600_WATCHDOG_DEFAULT<<4);
	HAL_I2C_Mem_Write(&hi2c1, AS5600_SLAVE_ADDRESS , AS5600_REGISTER_CONF_LOW, 1, &Data, 1, 100);
}
...
void Get_Magnet_Status(AS5600_TypeDef *Sensor){
	uint8_t Data[1];
	HAL_I2C_Mem_Read(&hi2c1, AS5600_SLAVE_ADDRESS , AS5600_REGISTER_STATUS, 1, &Data[1], 1, 100);
...
void Get_Angle(AS5600_TypeDef *Sensor){
	...
	readings[index] = Sensor->sensor_Angle;
